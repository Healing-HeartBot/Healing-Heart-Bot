...
function moodMapper(moods, hashtag) {
  const resultArray = [];
  const object = Object.entries(moods);
  for(let i = 0; i < object.length; i++) {
    if(object[i][0].includes(hashtag[0]) || object[i][1].includes(hashtag[0])) {
      resultArray.push(object[i][0]);
    }
    if(hashtag[1]) {
      if(object[i].includes(hashtag[1]) || object[i][1].includes(hashtag[1])) {
        resultArray.push(object[i][0]);
      }
    }
  }
